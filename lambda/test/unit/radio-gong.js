...
describe('Radio Gong Website', () => {
    xdescribe('#parsePlaylistBody()', () => {
        it('should parse playlist', () => {
            ...
            expect(playlist[0].cover, 'cover of current entry').to.equal(
                'https://sos-de-fra-1.exo.io/gong-tracks-bucket/00/00/76/93/_576x576_crop_center-center_60_line/43.jpg?mtime=20210304013210&focal=none&tmtime=20210304013514',
            );
    ...
    describe('#getPlaylist()', () => {
        it('should give songs playing', async function () {
            ...
            expect(playlist.playlist[0].title).to.be.a('string');
            expect(playlist.playlist[0].interpret).to.be.a('string');
            expect(playlist.playlist[0].cover).to.be.a('string');
        });
    });
    ...
    xdescribe('#parseTrafficBody()', () => {
        ...
        it('should parse 0 traffic messages and 1 traffic control', () => {
            ...
            expect(traffic.controls[0].msg, '1. traffic control').to.equal(
                'B8: Roßbrunn - Richtung Würzburg Mobiler Blitzanhänger "Enforcement-Trailer"',
            );
        ...
        it('should parse 0 traffic messages and 5 traffic controls', () => {
            ...
            expect(traffic.controls[0].msg, '1. traffic control').to.equal(
                'B19 Würzburg: Konrad-Adenauer-Brücke "Enforcement-Trailer" - Stadteinwärts',
            );
            ...
            expect(traffic.controls[1].msg, '2. traffic control').to.equal(
                'B19 Estenfeld unter der A7-Autobahnbrücke Richtung Schweinfurt',
            );
            ...
            expect(traffic.controls[2].msg, '3. traffic control').to.equal(
                'Kist Unter der Fußgängerbrücke in beide Richtungen',
            );
        ...
        it('should parse 1 traffic message and 1 traffic control', () => {
            ...
            expect(traffic.messages[0].msg, '1. traffic message').to.equal(
                'A7 Fulda Richtung Würzburg zwischen Dreieck Schweinfurt/Werneck und Gramschatzer Wald Staugefahr, Unfallaufnahme, Verkehr wird über den Standstreifen geleitet, Gasse für Rettungs- und Einsatzfahrzeuge bilden',
            );
            ...
            expect(traffic.controls).to.have.length(1);
            expect(traffic.controls[0].msg, '1. traffic control').to.equal(
                'A3 Würzburg-Nürnberg Ausfahrt Heidingsfeld Enforcement Trailer',
            );
        ...
        it('should parse 1 traffic message and 2 traffic controls', () => {
            ...
            expect(traffic.messages).to.have.length(1);
            expect(traffic.messages[0].msg, '1. traffic message').to.equal(
                'A7 Würzburg Richtung Fulda zwischen Hammelburg und Bad Kissingen/Oberthulba Stau, rechter Fahrstreifen gesperrt, defekter LKW, mittlerer Fahrstreifen gesperrt, benutzen Sie den linken Fahrstreifen, Reinigungsarbeiten, bis 18.02.2019 ca. 15:45 Uhr',
            );
            ...
            expect(traffic.controls[0].time, '1. traffic control').to.equal('08:19');
            expect(traffic.controls[1].msg, '2. traffic control').to.equal(
                'Blitzer Würzburg Nordtangente 30er Zone von Veitshöchheim kommend',
            );
        ...
        it('should parse 1 traffic message and 6 traffic controls', () => {
            ...
            expect(traffic.controls[2].msg, '3. traffic control').to.equal(
                'Würzburg Grombühlstr. bei Brauchbar 30er-Zone',
            );
            ...
            expect(traffic.controls[3].msg, '4. traffic control').to.equal(
                'Kürnach Ecke Hauptstraße Übergang Semmelstraße (20er Zone) ortsauswärts',
            );
            ...
            expect(traffic.controls[4].msg, '5. traffic control').to.equal(
                'Gemünden Richtung Lohr Ortsdurchfahrt (gegenüber Shell Tankstelle)',
            );
            ...
            expect(traffic.controls[5].msg, '6. traffic control').to.equal(
                'Ortseinfahrt Mainstockheim von Kitzingen kommend',
            );
        ...
        it('should parse 2 traffic messages and 1 traffic control', () => {
            ...
            expect(traffic.messages[0].msg, '1. traffic message').to.equal(
                'St2312 Waldbüttelbrunn zwischen Ausfahrt Waldbüttelbrunn und B 8 bei Höchberg in beiden Richtungen gesperrt, Unfall, Polizei ist vor Ort, Feuerwehr regelt den Verkehr, eine örtliche Umleitung ist eingerichtet',
            );
            ...
            expect(traffic.messages[1].msg, '2. traffic message').to.equal(
                'Unfall mit PKW und Roller auf der B8 von Höchberg kommend in Richtung Waldbüttelbrunn - Verkehrsbehinderung',
            );
            ...
            expect(traffic.controls).to.have.length(1);
            expect(traffic.controls[0].msg, '1. traffic control').to.equal(
                'Kitzingen Richtung Hörblach 80ger Zone (Blitzanhänger)',
            );
        ...
        it('should parse 2 traffic messages and 3 traffic controls', () => {
            ...
            expect(traffic.messages).to.have.length(2);
            expect(traffic.messages[0].msg, '1. traffic message').to.equal(
                'A3 Frankfurt Richtung Würzburg zwischen Würzburg/Kist und Würzburg-Heidingsfeld Unfall auf dem mittleren Fahrstreifen',
            );
            ...
            expect(traffic.messages[1].msg, '2. traffic message').to.equal(
                'B27 Karlstadt Richtung Würzburg zwischen Veitshöchheim und Rothofbrücke in Würzburg Gefahr durch Spanngurte auf dem rechten Fahrstreifen',
            );
            ...
            expect(traffic.controls[0].msg, '1. traffic control').to.equal(
                'Gaibach Richtung Volkach kurz bevor es bergab geht Enforcement Trailer',
            );
            ...
            expect(traffic.controls[2].msg, '3. traffic control').to.equal(
                'B8 Mainbernheim 60ger, von Kitzingen kommend',
            );
        ...
        it('should parse 3 traffic messages and 3 traffic controls', () => {
            ...
            expect(traffic.messages[0].msg, '1. traffic message').to.equal(
                'A3 Frankfurt Richtung Würzburg zwischen Würzburg/Kist und Würzburg-Heidingsfeld Gefahr durch einen Reifen auf allen Fahrstreifen, bitte vorsichtig fahren',
            );
            expect(traffic.messages[0].date, '1. traffic message').to.equal('18.02.2019');
            expect(traffic.messages[0].time, '1. traffic message').to.equal('14:40');
            expect(traffic.messages[1].msg, '2. traffic message').to.equal(
                'A3 Nürnberg Richtung Würzburg zwischen Ausfahrt Rottendorf und Würzburg/Randersacker Gefahr durch defekten LKW auf dem Standstreifen, langsam fahren',
            );
            ...
            expect(traffic.messages[2].msg, '3. traffic message').to.equal(
                'A7 Würzburg Richtung Fulda zwischen Hammelburg und Bad Kissingen/Oberthulba Stau, rechter Fahrstreifen gesperrt, defekter LKW, mittlerer Fahrstreifen gesperrt, benutzen Sie den linken Fahrstreifen, Reinigungsarbeiten, bis 18.02.2019 ca. 15:45 Uhr',
            );
